...
if (isNumeric!T || isComplex!T) {
    ...
    static Vector!(T, N) zero(size_t dim) {
        static if (N == 0) {
            auto result = Vector!(T, N)(dim);
            static if (isComplex!T) {
                result._components[] = T(0, 0);
            } else {
                result._components[] = 0;
    ...
    Vector!(T, N) hadamard(const Vector!(T, N) other) const {
        ...
        static if (isSimd!(typeof(_components))) {
            ...
            result._components[] = this._components[] * other._components[];
        }
        return result;
    }
...
unittest {
    ...
    // Test basic double vectors (ensure backwards compatibility)
    {
        ...
        assert(v1.magnitude() == 3.0, "1D vector magnitude failed, expected: 3.0, got: " ~ to!string(v1.magnitude()));

        // Test 2D vector
        auto v2a = Vector2D!double(3.0, 4.0);
        auto v2b = Vector2D!double(1.0, 2.0);
        assert(v2a.magnitude() == 5.0, "2D vector magnitude failed, expected: 5.0, got: " ~ to!string(v2a.magnitude()));
        assert((v2a + v2b)[0] == 4.0, "2D vector addition x-component failed, expected: 4.0, got: " ~ to!string((v2a + v2b)[0]));
        assert((v2a + v2b)[1] == 6.0, "2D vector addition y-component failed, expected: 6.0, got: " ~ to!string((v2a + v2b)[1]));

        // Test 3D vector
        auto v3 = Vector3D!double(1.0, 2.0, 2.0);
        assert(v3.magnitude() == 3.0, "3D vector magnitude failed, expected: 3.0, got: " ~ to!string(v3.magnitude()));
        auto unit = v3.unit();
        assert(abs(unit.magnitude() - 1.0) < 1e-10, "Unit vector magnitude deviation: " ~ to!string(abs(unit.magnitude() - 1.0)));

        // Test dynamic vector
        auto dv1 = DynamicVector!double([1.0, 2.0, 3.0]);
        auto dv2 = DynamicVector!double([4.0, 5.0, 6.0]);
        assert(dv1.length == 3, "Dynamic vector dimension failed, expected: 3, got: " ~ to!string(dv1.length));
        assert(dv1.dot(dv2) == 32.0, "Dynamic vector dot product failed, expected: 32.0, got: " ~ to!string(dv1.dot(dv2)));

            // Test scalar operations
            auto basicScaled = dv1 * 2.0;
            assert(basicScaled[0] == 2.0, "Vector scalar multiplication failed for index 0, expected: 2.0, got: " ~ to!string(basicScaled[0]));
            assert(basicScaled[1] == 4.0, "Vector scalar multiplication failed for index 1, expected: 4.0, got: " ~ to!string(basicScaled[1]));
            assert(basicScaled[2] == 6.0, "Vector scalar multiplication failed for index 2, expected: 6.0, got: " ~ to!string(basicScaled[2]));

            // Test vector operations
            auto basicSum = dv1 + dv2;
            assert(basicSum[0] == 5.0, "Vector addition failed for index 0, expected: 5.0, got: " ~ to!string(basicSum[0]));
            assert(basicSum[1] == 7.0, "Vector addition failed for index 1, expected: 7.0, got: " ~ to!string(basicSum[1]));
            assert(basicSum[2] == 9.0, "Vector addition failed for index 2, expected: 9.0, got: " ~ to!string(basicSum[2]));
        ...
        assert(sumComponents == 6.0, "Vector component sum failed, expected: 6.0, got: " ~ to!string(sumComponents));
        ...
        // Complex number tests
        {
            ...
            assert(cv2.magnitudeSquared().isClose(10.0), "Complex vector magnitude squared failed, expected: 10.0, got: " ~ to!string(cv2.magnitudeSquared())); // |1+i|² + |2+2i|² = 2 + 8 = 10

            // Test dot product with complex numbers
            auto cv2b = Vector2D!C(C(1, 0), C(0, 1));
            auto dot_result = cv2b.dot(cv2b);
            assert(dot_result.re.isClose(2.0), "Complex dot product real part failed, expected: 2.0, got: " ~ to!string(dot_result.re)); // (1-0i)(1+0i) + (0-i)(0+i) = 1 + 1 = 2
            assert(dot_result.im.isClose(0.0), "Complex dot product imaginary part failed, expected: 0.0, got: " ~ to!string(dot_result.im));

            // Test unit vector with complex components
            auto cv2_unit = cv2.unit();
            assert((cv2_unit.magnitude() - 1.0).abs < 1e-10, "Complex unit vector magnitude deviation: " ~ to!string((cv2_unit.magnitude() - 1.0).abs));

            // Dynamic complex vector
            auto dcv = DynamicVector!C([C(1, 1), C(2, 2), C(3, 3)]);
            assert(dcv.length == 3, "Dynamic complex vector dimension failed, expected: 3, got: " ~ to!string(dcv.length));
            ...
            assert(hadamard_prod[0] == C(2, 2), "Complex Hadamard product[0] failed, expected: 2+2i, got: " ~ to!string(hadamard_prod[0]));
            assert(hadamard_prod[1] == C(6, 6), "Complex Hadamard product[1] failed, expected: 6+6i, got: " ~ to!string(hadamard_prod[1]));

            // Test arithmetic operations
            auto complexSum = cv2c + cv2d;
            assert(complexSum[0] == C(3, 1), "Complex vector addition[0] failed, expected: 3+i, got: " ~ to!string(complexSum[0]));
            assert(complexSum[1] == C(5, 2), "Complex vector addition[1] failed, expected: 5+2i, got: " ~ to!string(complexSum[1]));

            auto complexScaled = cv2c * C(2, 1); // Scale by 2+i
            assert(complexScaled[0] == C(1, 3), "Complex scalar multiplication[0] failed, expected: 1+3i, got: " ~ to!string(complexScaled[0]));
            assert(complexScaled[1] == C(2, 6), "Complex scalar multiplication[1] failed, expected: 2+6i, got: " ~ to!string(complexScaled[1]));
        }
        ...
        // Double SIMD tests
        {
            ...
            static if (is(double2)) {
                ...
                assert(abs(simd_sum[0] - 6.0) < double.epsilon && 
                       ...
                       "SIMD vector addition failed, expected: [6.0, 8.0], got: [" ~ to!string(simd_sum[0]) ~ ", " ~ to!string(simd_sum[1]) ~ "]");

                auto simd_mul = dv2_simd * 2.0;
                assert(abs(simd_mul[0] - 4.0) < double.epsilon && 
                       abs(simd_mul[1] - 6.0) < double.epsilon,
                       "SIMD scalar multiplication failed, expected: [4.0, 6.0], got: [" ~ to!string(simd_mul[0]) ~ ", " ~ to!string(simd_mul[1]) ~ "]");

                auto simd_div = dv2_simd / 2.0;
                assert(abs(simd_div[0] - 1.0) < double.epsilon && 
                       abs(simd_div[1] - 1.5) < double.epsilon,
                       "SIMD scalar division failed, expected: [1.0, 1.5], got: [" ~ to!string(simd_div[0]) ~ ", " ~ to!string(simd_div[1]) ~ "]");
                ...
                assert(abs(hadamard[0] - 8.0) < double.epsilon && 
                       ...
                       "SIMD Hadamard multiplication failed, expected: [8.0, 15.0], got: [" ~ to!string(hadamard[0]) ~ ", " ~ to!string(hadamard[1]) ~ "]");
            }
            ...
            static if (is(double4)) {
                ...
                assert(abs(simd_sum3[0] - 5.0) < double.epsilon && 
                       ...
                       to!string(simd_sum3[0]) ~ ", " ~ to!string(simd_sum3[1]) ~ ", " ~ to!string(simd_sum3[2]) ~ "]");
                ...
                assert(abs(simd_mul3[0] - 2.0) < double.epsilon && 
                       ...
                       to!string(simd_mul3[0]) ~ ", " ~ to!string(simd_mul3[1]) ~ ", " ~ to!string(simd_mul3[2]) ~ "]");
        ...
        // Integer vector tests
        {
            ...
            assert(iv2.magnitude() == 5, "Integer vector magnitude failed, expected: 5, got: " ~ to!string(iv2.magnitude()));

            auto iv3 = Vector3D!int(2, 3, 6);
            assert(iv3.dot(iv3) == 49, "Integer vector dot product failed, expected: 49, got: " ~ to!string(iv3.dot(iv3)));

            // Test arithmetic
            auto iv2b = Vector2D!int(1, 2);
            auto intSum = iv2 + iv2b;
            assert(intSum[0] == 4, "Integer vector addition[0] failed, expected: 4, got: " ~ to!string(intSum[0]));
            assert(intSum[1] == 6, "Integer vector addition[1] failed, expected: 6, got: " ~ to!string(intSum[1]));

            auto intScaled = iv2 * 2;
            assert(intScaled[0] == 6, "Integer vector scalar multiplication[0] failed, expected: 6, got: " ~ to!string(intScaled[0]));
            assert(intScaled[1] == 8, "Integer vector scalar multiplication[1] failed, expected: 8, got: " ~ to!string(intScaled[1]));
        }
        ...
        // Float vector tests with SIMD
        {
            ...
            assert(abs(fv2.magnitude() - 5.0f) < float.epsilon,
                   "Float vector magnitude failed, expected: 5.0, got: " ~ to!string(fv2.magnitude()));

            auto fv2b = Vector2D!float(1.0f, 2.0f);
            auto sum = fv2 + fv2b;
            assert(sum[0] == 4.0f, "Float vector addition[0] failed, expected: 4.0, got: " ~ to!string(sum[0]));
            assert(sum[1] == 6.0f, "Float vector addition[1] failed, expected: 6.0, got: " ~ to!string(sum[1]));
            ...
            static if (is(float2)) {
                ...
                assert(simd_sum[0] == 6.0f && simd_sum[1] == 8.0f,
                       "Float SIMD vector addition failed, expected: [6.0, 8.0], got: [" ~ to!string(simd_sum[0]) ~ ", " ~ to!string(simd_sum[1]) ~ "]");

                auto simd_mul = fv2_simd * 2.0f;
                assert(simd_mul[0] == 4.0f && simd_mul[1] == 6.0f,
                       "Float SIMD scalar multiplication failed, expected: [4.0, 6.0], got: [" ~ to!string(simd_mul[0]) ~ ", " ~ to!string(simd_mul[1]) ~ "]");

                auto simd_div = fv2_simd / 2.0f;
                assert(simd_div[0] == 1.0f && simd_div[1] == 1.5f,
                       "Float SIMD scalar division failed, expected: [1.0, 1.5], got: [" ~ to!string(simd_div[0]) ~ ", " ~ to!string(simd_div[1]) ~ "]");

                auto simd_dot = fv2_simd.dot(fv2b_simd);
                assert(simd_dot == 23.0f,
                       "Float SIMD dot product failed, expected: 23.0, got: " ~ to!string(simd_dot)); // 2*4 + 3*5 = 23
            }
            ...
            static if (is(float4)) {
                ...
                assert(simd_dot3 == 32.0f,
                       "Float SIMD 3D dot product failed, expected: 32.0, got: " ~ to!string(simd_dot3)); // 1*4 + 2*5 + 3*6 = 32
            }
            ...
            static if (is(float2)) {
                ...
                assert(hadamard[0] == 8.0f && hadamard[1] == 15.0f,
                       "Float SIMD Hadamard multiplication failed, expected: [8.0, 15.0], got: [" ~ to!string(hadamard[0]) ~ ", " ~ to!string(hadamard[1]) ~ "]");

                auto hadamard_div = hv2a.hadamardDiv(hv2b);
                assert(abs(hadamard_div[0] - 0.5f) < float.epsilon,
                       "Float SIMD Hadamard division[0] failed, expected: 0.5, got: " ~ to!string(hadamard_div[0]));
                assert(abs(hadamard_div[1] - 0.6f) < float.epsilon,
                       "Float SIMD Hadamard division[1] failed, expected: 0.6, got: " ~ to!string(hadamard_div[1]));
            }
        }
        ...
        // Test serialization of vectors
        {
            ...
            // Dynamic vector
            {
                ...
                assert(dv_ser.length == dv_restored.length, 
                       "Dynamic vector dimension mismatch after serialization, expected: " ~ to!string(dv_ser.length) ~ 
            ...
            // Test reference fromBytes
            {
                ...
                assert(v1_bytes_read == 1 + long.sizeof + double.sizeof, 
                       "1D vector bytes read mismatch, expected: " ~ to!string(1 + long.sizeof + double.sizeof) ~ ", got: " ~ to!string(v1_bytes_read));
                assert(v1_ref == v1_ser, format("1D vector reference deserialization failed\nExpected: %s\nGot: %s", 
                    v1_ser, v1_ref));
                ...
                assert(v2_bytes_read == 1 + long.sizeof + double.sizeof * 2, 
                       "2D vector bytes read mismatch, expected: " ~ to!string(1 + long.sizeof + double.sizeof * 2) ~ ", got: " ~ to!string(v2_bytes_read));
                assert(v2_ref == v2_ser, format("2D vector reference deserialization failed\nExpected: %s\nGot: %s", 
                    v2_ser, v2_ref));
                ...
                assert(v3_bytes_read == 1 + long.sizeof + double.sizeof * 3, 
                       "3D vector bytes read mismatch, expected: " ~ to!string(1 + long.sizeof + double.sizeof * 3) ~ ", got: " ~ to!string(v3_bytes_read));
                assert(v3_ref == v3_ser, format("3D vector reference deserialization failed\nExpected: %s\nGot: %s", 
                    v3_ser, v3_ref));
                ...
                assert(dv_bytes_read == 1 + long.sizeof + double.sizeof * dv_ser.length, 
                       "Dynamic vector bytes read mismatch, expected: " ~ to!string(1 + size_t.sizeof + double.sizeof * dv_ser.length) ~ 
                       ", got: " ~ to!string(dv_bytes_read));
                assert(dv_ref == dv_ser, format("Dynamic vector reference deserialization failed\nExpected: %s\nGot: %s", 
                    dv_ser, dv_ref));
                assert(dv_ref.length == dv_ser.length, 
                       "Dynamic vector reference dimension mismatch, expected: " ~ to!string(dv_ser.length) ~ 
